...
def main(out=sys.stdout):
    ...
    w("*%% Raster calls for IPP keywords. Declaring the cups-raster *cupsFilter2 is\n")
    w("*%% what forces our route: CUPS turns each *cupsFilter2 into an edge into\n")
    w("*%% printer/<queue>, so cgpdftoraster feeds rastertobrother rather than\n")
    w("*%% reaching the queue on its own.\n")
    w('*cupsFilter2: "application/vnd.cups-raster image/pwg-raster 20 rastertobrother"\n')

    w("\n*%% The second edge is for iOS. AirPrint clients send Apple Raster, and\n")
    w("*%% libcups reads it transparently -- cupsRasterOpen() in read mode detects\n")
    w("*%% the UNIRAST sync word (CUPS_RASTER_HAVE_APPLERASTER), so rastertobrother\n")
    w("*%% needs no code for it. Without this edge CUPS still advertises image/urf,\n")
    w("*%% because the image/* wildcard in apple.convs routes it through\n")
    w("*%% cgimagetopdf -- which cannot read Apple Raster, so the job dies there.\n")
    w("*%%\n")
    w("*%% Cost 40 is chosen to sit inside a window, not picked for taste:\n")
    w("*%%   image/urf direct         = 40   vs via cgimagetopdf 25+100+20 = 145\n")
    w("*%%   application/pdf as today = 120  vs via cgpdftoraster+urf 100+40 = 140\n")
    w("*%% so URF takes this edge while PDF stays on the tested raster route.\n")
    w('*cupsFilter2: "image/urf image/pwg-raster 40 rastertobrother"\n')

    w("\n*%% What makes the queue AirPrint rather than Mopria when it is shared.\n")
    w("*%% iOS looks for a URF key in the Bonjour TXT record and ignores anything\n")
    w("*%% without one -- the printer itself has no urf-supported attribute, which\n")
    w("*%% is the whole reason this driver exists, so the Mac has to supply it.\n")
    w("*%% DM1 is simplex-only, which is honest: the DCP-T420W has no duplexer.\n")
    w('*cupsUrfSupported: "CP1,IS1,PQ3-4-5,RS300-600,SRGB24,W8,DM1,FN3,V1.4"\n')
    w("\n")
